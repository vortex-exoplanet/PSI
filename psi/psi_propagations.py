...
def prop_image(wf_post_coro_, ncpa_, correction_, prop_, coro_, wv=0, noise_=True, n_pho=8.88e+04/1000, background_subtract=False):
    ...
    GOX modifications:
    -------------------
        - wf_post_coro = wf_post_coro_.copy() and then use wf_post_coro. Otherwise the 'external' field get modified...
	...
	wf_post_coro.electric_field *= np.exp(1j * ncpa_) * np.exp(1j * correction_)# * np.exp(1j * wv) # Add NCPA and correction
	if coro_ is None:
...
def prop_wf(wf_post_ao_, aperture_, prop_, coro_, recon_m, trans_m):
	'''
	TODO: remove subtraction by aperture_ (create artificial phase wrapping and not apparent reason for it)
	'''
	#
