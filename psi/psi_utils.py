...
def load_file(folder, file, grid_size, aperture_):
    ncpa_ = fits.getdata(os.path.join(folder, file)) * 2 * np.pi / wavelength * 1e3 * 1e-6	# mm * rad/um * m/mm * um/m
    ncpa_ = process_screen(ncpa_, grid_size_, aperture_, rotate=True)   # The NCPA from Tibor come in 512 so need to be resized.
    return ncpa_

def loadNCPA(aperture_, size_, file_="DIFF_rep_1_field_0.fits", folder_="/Users/matt/Documents/METIS/TestArea/fepsi/NCPA_Tibor/", wavelength_=3e-6):
    ...
    ncpa_ = fits.getdata(os.path.join(folder_, file_)) * 2 * np.pi / wavelength_ * 1e3 * 1e-6	# mm * rad/um * m/mm * um/m
...
def load_and_process(file, in_fold, mask_pup, grid, pow=1e3, wavelength=1.0, piston=True):
	...
	if remove_piston == True:
		phase_pupil = remove_piston(phase_pupil, mask_pup)
	phase_residual_ao = Field(phase_pupil.ravel(), grid)
	wf_post_ = Wavefront(np.exp(1j * phase_residual_ao) * mask_pup, wavelength)
